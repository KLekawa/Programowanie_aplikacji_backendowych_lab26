...
public class CreatePersonDtoValidator : AbstractValidator<CreatePersonDto>
{
    ...
    public CreatePersonDtoValidator(ICompanyRepositoryAsync companyRepository)
    {
        ...
        RuleFor(x => x.LastName)
            .NotEmpty().WithMessage("Naziwsko jest wymagane")
            .MaximumLength(200).WithMessage("Nazwisko nie może przekraczać 200 znaków")
            .Matches(@"^[\p{L}\s\-]+$").WithMessage("Nazwisko zawiera nie poprawne dane");
        ...
        RuleFor(x => x.Phone)
            .Matches(@"^[^\+?[1-9]\d{6,14}$]").WithMessage("Nieprawidłowy format numeru telefonu.")
